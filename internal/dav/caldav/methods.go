...
func (h *Handlers) HandleACL(w http.ResponseWriter, r *http.Request) {
	pr := common.MustPrincipal(r.Context())

	body, err := io.ReadAll(io.LimitReader(r.Body, 1<<20))
	if err != nil {
		h.logger.Error().Err(err).Msg("failed to read ACL body")
		http.Error(w, "bad request", http.StatusBadRequest)
		return
	}
	_ = r.Body.Close()
	...
	if len(body) > 0 {
		h.logger.Debug().
			Str("user", pr.UserID).
			Str("path", r.URL.Path).
			Msg("ACL modification attempted - denied")
	...
	if owner != "" && calURI != "" && len(rest) == 0 {
		...
		if err != nil {
			h.logger.Error().Err(err).
				Str("owner", owner).
				Str("calendar", calURI).
				Msg("failed to resolve calendar in ACL")
			http.NotFound(w, r)
			return
		}
		...
		acl = common.BuildReadOnlyACL(r, h.basePath, calURI, calOwner, h.aclProv)
	} else {
		ownerUID := owner
		if ownerUID == "" {
			ownerUID = pr.UserID
		...
		acl = common.BuildReadOnlyACL(r, h.basePath, "", ownerUID, h.aclProv)
	}
	...
	if acl == nil {
		h.logger.Error().
			Str("user", pr.UserID).
			Str("path", r.URL.Path).
			Msg("failed to build ACL response")
	...
	if err := enc.Encode(acl); err != nil {
		h.logger.Error().Err(err).Msg("failed to encode ACL response")
	}
}
